    ...
    try {
      ...
      if (path === "/api/search") {
        ...
        if (!type || type === "movie") {
          if (env.TMDB_API_KEY) {
            try {
              let movies = await searchMovies(query, page, env.TMDB_API_KEY);
        ...
        if (!type || type === "book") {
          try {
            console.log("searching book: ", query);
      ...
      if (path === "/api" || path === "/api/") {
        const documentation = {
          ...
          version: "1.0.0",
          description:
            "Search for movies using TMDB and books using OpenLibrary",
          endpoints: {
            "/api/search": {
              method: "GET",
              ...
              parameters: {
                ...
                limit:
                  "Results per page for books (optional, default: 5, max: 100)",
              },
